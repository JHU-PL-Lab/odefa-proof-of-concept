...
sig
  ...
  val pretty_state : M.state -> string
  val pretty_input : M.input -> string
  val pretty_symbol : M.symbol -> string
end;;

module Make(S : Spec) =
struct
  let dot_string_of_umisa
        ?transition_formatter:
          (transition_formatter=
            fun pop_str input_str pushes_str ->
              pop_str ^ ";" ^ input_str ^ ";" ^ pushes_str)
        umisa
        : string =
    ...
    (foreach (S.M.transitions_of_umisa umisa) @@
      function (in_state, input_option, pop_option, out_state, pushes) ->
        ...
        let input_string = Option.map_default S.pretty_input "-" input_option in
