...
struct
  ...
  let lookup g init_x init_acl0 =
    let rec lookup' visits x acl0 lookups context : Value_set.t =
      ...
      then
        (logger `debug
           "Visit occurrence check has succeeded; yielding empty result.";
         Value_set.empty)
      else
        let visits' = Visit_set.add visit visits in 
          ...
          (fun acl1 ->
                         ...
                         begin
                           ...
                           | Value_function(_) ->
                             (* Rule #2 applies: we've found
                                ...
                                closure. *)
                             lookup' visits' x1 acl1 lookups_t context
                           | _ ->
                 ...
                 begin
                   if x = x_here then
                     (* Rule #4 applies: we're looking for a function's
                        ...
                        site. *)
                     lookup' visits' x_from acl1 lookups @@ S.pop context
                   else
             ...
             | Exit_clause(x_here,x_from,site) ->
               if x_here = x then
                 ...
                 lookup' visits' x_from acl1 lookups @@ S.push site context
               else
    ...
    in
    lookup' Visit_set.empty init_x init_acl0 [] S.empty
  ;;
